...
import { createJob, GetSingleJob, updateJob, reset } from "../../../features/job/jobSlice";
...
const UpdateJob = () => {
	  ...
	  useEffect(() => {
		if (singleJob) {
		  setFormData({
			...
			place: singleJob.place,
			benefits: singleJob.benefits,
			skillsRequired: singleJob.skillsRequired,
    ...
    const onSubmit = async (e) => {
		...
		try {
			await dispatch(updateJob({ jobData, jobId:id }));
			
			alert("Job Updated")
			navigate(-1);
		   
		  } catch (error) {
			console.error('Error updatibg job:', error);
